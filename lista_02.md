...
class Veiculo {
    ...
    calcularConsumo(km) {
        return "Método a ser implementado nas subclasses";
    }
}
...
function calcularTempoPouso(velocidadeInicial, desaceleracao, velocidadeSegura, tempoMaximo) {
    if (desaceleracao <= 0) {
        return "Erro: A desaceleração deve ser positiva.";
    }
    ...
    if (tempoNecessario < 0) {
        return "A sonda já está na velocidade segura.";
    } else if (tempoNecessario > tempoMaximo) {
        return `Aviso: Tempo necessário (${tempoNecessario.toFixed(2)}s) excede o limite máximo permitido!`;
    } else {
        return `Tempo necessário para pouso seguro: ${tempoNecessario.toFixed(2)} segundos.`;
    }
}
```
______
...
let impactoFinanceiro = multiplicarMatrizesInvestimento(investimentosAno1, fatoresCrescimento);
